...
struct Node {
    ...
    Node(int* arr, int start, int end) {
        ...
        //Code aqui :D
        //cout <<"El start " << start <<" y el final "<<end<< endl;
        if (final_index == 0) {
            children[0] = new Node(arr, 0, final_index-1);
        }
        if (final_index != 0) {
            /*if ((final_index + 1) % 2 == 0) {
                final_index = (final_index + 1) / 2;
                cout << final_index<<endl;
            }else {
                final_index = ((final_index) / 2) + 1;
                cout << final_index << endl;
            }*/
            children[0] = new Node(arr, 0, (final_index)/2);
            int resultado = 0;
            for (int* p = arr; p <= arr + final_index; p++) {
                resultado += *p;
            }
            //int valor = arr[final_index];
            children[0]->sum = resultado;
            cout << children[0]->sum << endl;

            //children[1] = new Node(arr, 0, final_index + 1);
        }
        /*children[1] = new Node(arr, 0, 0);
        children[1]->sum = 9;
        cout << children[1]->sum << endl;*/
