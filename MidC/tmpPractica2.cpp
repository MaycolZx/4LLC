...
struct Node {
    ...
    Node(int* arr, int start, int end) {
        ...
        if (start_index == final_index) {
            sum = *(arr + start_index);//Aritmetica de punteros
            cout << sum << endl;
            return;
        }
        children[0] = new Node(arr, start_index, ((final_index - start_index) / 2) + start_index);
        children[1] = new Node(arr, ((final_index - start_index) / 2) + start_index + 1, final_index);
